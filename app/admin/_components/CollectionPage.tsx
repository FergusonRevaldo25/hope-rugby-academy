...
export default function CollectionPage({ collection }: { collection: string }) {
  ...
  async function load() {
    ...
    setError("");
    try {
      const r = await fetch(`/api/admin/content/${collection}`, {
        cache: "no-store",
      });
  ...
  if (!config)
    return <div className="p-8 text-red-400">Unknown collection.</div>;

  async function remove(id: string) {
    if (!confirm("Delete this item? This cannot be undone.")) return;
    const r = await fetch(`/api/admin/content/${collection}/${id}`, {
      method: "DELETE",
    });
    if (!r.ok) {
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-white/45 text-sm mt-1">
            Create, edit, publish and reorder {config.title.toLowerCase()}.
          </p>
        </div>
        <button
          onClick={() =>
            setEditing({ id: "", order: items.length, published: true })
          }
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/10 bg-[#12121a]">
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                >
                  ...
                  <td className="p-4 text-right">
                    <button
                      ...
                      className="p-2 rounded-lg hover:bg-white/10 mr-1"
                    >
              ...
              {!items.length && (
                <tr>
                  <td colSpan={4} className="p-10 text-center text-white/35">
                    No items yet.
                  </td>
                </tr>
              )}
        ...
        <form onSubmit={save} className="p-6 space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold">
              {form.id ? "Edit" : "Add"} {config.singular}
            </h2>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="grid gap-4">
                ...
                return (
                  <div key={f.key}>
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      />
                      <label className="shrink-0 inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-white/10 hover:bg-white/15 cursor-pointer text-sm">
                    ...
                    {form[f.key] && (
                      <img
                        src={form[f.key]}
                        alt=""
                        className="mt-3 w-28 h-20 object-cover rounded-lg border border-white/10"
                      />
                    )}
                  </div>
                );
              const common = {
                value: inputValue(f, form[f.key]),
                required: f.required,
            ...
            <div className="grid grid-cols-2 gap-4">
              <label>
                <span className="block text-sm text-white/60 mb-2">Order</span>
                <input
                  className="input"
                  type="number"
                  value={form.order ?? 0}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              disabled={saving}
              className="px-5 py-2 rounded-lg bg-red-600 hover:bg-red-500 font-semibold"
            >
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
